...
const FriendsPage = () => {
  ...
  const loadFriends = async () => {
    try {
      ...
      const friendsList = contacts.map((contact) => ({
        ...
        phoneNumber: contact.friend.phoneNumber,
      }));
  ...
  const loadInvitations = async () => {
    try {
      ...
      const invitesList = invitations.map((invite) => {
        const firstName = invite.sender.firstName || "";
        const lastName = invite.sender.lastName || "";

        return {
          id: invite.invitationId,
          name: `${firstName} ${lastName}`.trim(),
          senderId: invite.sender.userId,
        };
      });
  ...
  const handleSearchNewFriends = async () => {
    ...
    setIsSearching(true);
    try {
      const results = await contactsAPI.searchNewContact(searchQuery);
  ...
  const handleRejectInvite = async (inviteId) => {
    try {
      ...
      console.error("Помилка відхилення запрошення:", error);
    }
  };

  const handleDeleteFriend = async (friendId) => {
    try {
      await contactsAPI.deleteContact(friendId);
  ...
  const handleMemberDeleteClick = (member, group) => {
    ...
    setDeleteTarget({
      type: "member",
      data: { ...member, groupId: group.id, groupName: group.name },
    });
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      switch (deleteTarget.type) {
        case "friend":
          await handleDeleteFriend(deleteTarget.data.id);
          break;
        case "group":
        ...
        case "member":
          if (deleteTarget.data.id === MY_USER_ID) {
            setGroups((prevGroups) =>
              prevGroups.filter((g) => g.id !== deleteTarget.data.groupId)
            );
          } else {
            setGroups((prevGroups) =>
    ...
    switch (deleteTarget.type) {
      ...
      case "member":
        if (id === MY_USER_ID) {
          return (
            <>
              Ви впевнені, що хочете вийти з групи <strong>{groupName}</strong>?
            </>
          );
        }
        return (
          <>
            Ви впевнені, що хочете видалити <strong>{name}</strong> з групи{" "}
    ...
    <>
      <div className="bg-[#B6CDFF] w-[992px] h-[904px] mt-[20px] mb-[20px] mx-auto rounded-3xl flex flex-row justify-center gap-[28px] p-[28px]">
        ...
        <div className="bg-white w-[454px] h-[848px] rounded-2xl pt-[28px] flex flex-col items-center">
          <h2 className="w-[382px] text-[32px] text-[#021024] font-semibold">
            Мої запрошення
          </h2>
          {invites.length > 0 ? (
              ...
              {invites.map((invite) => (
                ...
                >
                  ...
                  <div className="absolute bottom-[20px] right-[20px] flex gap-[12px]">
                    <button
                      onClick={() =>
                        handleAcceptInvite(invite.id, invite.senderId)
                      }
